...
window.addEventListener('message', async e => { if (e.data.action === 'open') { document.getElementById('app').classList.remove('hidden'); await fetchData(); show('dashboard') } })
...
function show(page){ currentPage = page; renderMenu(); document.getElementById('content').innerHTML = globalSearch() + (({dashboard,citizens,cases,wanted,vehicles,suspicious,laws}[page] || dashboard)()) }
...
function searchSuggestions(value){
  ...
  const results = []
  state.citizens.forEach(c => { if (`${c.name} ${c.citizenid}`.toLowerCase().includes(q)) results.push({type:'citizen', title:c.name, sub:c.citizenid + ' • ' + c.phone, action:`citizenDetails('${c.citizenid}')`}) })
  state.vehicles.forEach(v => { if (`${v.plate} ${v.owner_name||v.owner||''} ${v.vehicle||''}`.toLowerCase().includes(q)) results.push({type:'vehicle', title:v.plate, sub:(v.owner_name||v.owner||'') + ' • ' + (v.vehicle||''), action:`vehicleDetails('${v.plate}')`}) })
  box.innerHTML = results.slice(0, 8).map(r => `<div class="suggestion" onclick="${r.action}; hideSuggestions()"><b>${r.type === 'citizen' ? '👤' : '🚗'} ${r.title}</b><small>${r.sub}</small></div>`).join('') || '<div class="suggestion empty">لا توجد نتائج</div>'
