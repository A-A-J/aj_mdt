...
function laws(){
  ...
  return `<header><div><h1>${t('laws')}</h1><p>تعريفات القضايا والمخالفات المرورية</p></div></header><div class="laws-page"><section class="panel law-panel"><div class="law-panel-header"><h2>القضايا</h2>${can('manage_laws') ? `<button class="primary" onclick="openLawModal('قضية')"><i class="fa fa-plus"></i> إضافة قضية</button>` : ''}</div>${lawTable('caseDefinitionsTable', caseLaws, 'قضية')}</section><section class="panel law-panel"><div class="law-panel-header"><h2>المخالفات المرورية</h2>${can('manage_laws') ? `<button class="primary" onclick="openLawModal('مخالفة')"><i class="fa fa-plus"></i> إضافة مخالفة</button>` : ''}</div>${lawTable('trafficDefinitionsTable', trafficLaws, 'مخالفة')}</section></div>${lawModals()}`
}
function lawTable(id, list, type){
  const rows = (list || []).map((l, i) => `<tr><td>${lawNumber(i)}</td><td>${esc(l.title_ar || '-')}</td><td>${lawActionLabel(type)}</td><td>${lawDurationLabel(l)}</td><td class="law-actions">${can('manage_laws') ? `<button class="icon-btn" title="تحرير" onclick="openLawModal('${type}', ${Number(l.id)})"><i class="fa fa-edit"></i></button>` : ''}${can('manage_laws') ? `<button class="icon-btn danger-btn" title="حذف" onclick="deleteLawDefinition(${Number(l.id)})"><i class="fa fa-trash"></i></button>` : ''}<button class="icon-btn" title="معلومات" onclick="showLawDefinitionInfo(${Number(l.id)})"><i class="fa fa-info-circle"></i></button></td></tr>`)
  return table(id, ['#', type === 'قضية' ? 'نوع القضية' : 'نوع المخالفة', 'الإجراء', 'المدة', 'الإجراءات'], rows)
}
function lawModals(){ return `<div id="lawModal" class="modal hidden"><div class="modal-box law-modal-box"><h2 id="lawModalTitle">إضافة تعريف</h2><input id="lawName" placeholder="اسم التعريف"><input id="lawEnglish" placeholder="English name / اختياري"><div id="lawCaseFields" class="law-field-block hidden"><label>مدة السجن بالشهور</label><input id="lawJail" type="number" min="0" placeholder="مثال: 500"></div><div id="lawTrafficFields" class="law-field-block hidden"><label>قيمة المخالفة بالريال</label><input id="lawFine" type="number" min="0" placeholder="مثال: 3000"></div><div class="case-actions"><button onclick="closeLawModal()"><i class="fa fa-times"></i> إلغاء</button><button class="primary" onclick="saveLawDefinition()"><i class="fa fa-save"></i> حفظ</button></div></div></div><div id="lawInfoModal" class="modal hidden"><div class="modal-box law-modal-box"><h2>معلومات التعريف</h2><div id="lawInfoBody" class="law-info-body"></div><div class="case-actions"><button class="primary" onclick="closeLawInfoModal()"><i class="fa fa-check"></i> تم</button></div></div></div>` }
...
function closeLawModal(){ document.getElementById('lawModal')?.classList.add('hidden'); editingLawId = null; editingLawType = null }
async function saveLawDefinition(){
  ...
  const payload = { id: editingLawId, title_ar: name, title_en: val('lawEnglish').trim(), type: editingLawType, fine: editingLawType === 'قضية' ? 0 : Number(val('lawFine') || 0), jail: editingLawType === 'قضية' ? Number(val('lawJail') || 0) : 0 }
...
function showLawDefinitionInfo(id){
  ...
  document.getElementById('lawInfoBody').innerHTML = `<div class="law-info-row"><span>الاسم</span><b>${esc(law.title_ar || '-')}</b></div><div class="law-info-row"><span>النوع</span><b>${esc(law.type || '-')}</b></div><div class="law-info-row"><span>الإجراء</span><b>${lawActionLabel(law.type)}</b></div><div class="law-info-row"><span>المدة / القيمة</span><b>${lawDurationLabel(law)}</b></div><div class="law-info-row"><span>تمت الإضافة بواسطة</span><b>${esc(law.created_by || '-')}</b></div><div class="law-info-row"><span>تاريخ الإضافة</span><b>${esc(law.created_at || '-')}</b></div><div class="law-info-row"><span>آخر تحرير بواسطة</span><b>${esc(law.updated_by || '-')}</b></div><div class="law-info-row"><span>تاريخ آخر تحديث</span><b>${esc(law.updated_at || '-')}</b></div>`
  document.getElementById('lawInfoModal').classList.remove('hidden')
}
function closeLawInfoModal(){ document.getElementById('lawInfoModal')?.classList.add('hidden') }
