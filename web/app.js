...
function renderHeader(){
  ...
  if(officerInfo){
    officerInfo.innerHTML = `<img src="${esc(officer.image || fallbackAvatarSmall)}" onerror="this.src='${fallbackAvatarSmall}'"><div><b>${esc(officer.name || 'MDT Officer')}</b><small>${esc(officer.job || 'Mobile Data Terminal')}</small></div>`
  }
  if(headerSearch){
...
function dashboard(){ return `<header><h1>${t('dashboard')}</h1><p>MDT Control Panel</p></header><div class="stats"><div class="card"><small>${t('citizens')}</small><b>${state.citizens.length}</b></div><div class="card"><small>${t('cases')}</small><b>${state.cases.length}</b></div><div class="card"><small>${t('wanted')}</small><b>${state.wanted.length}</b></div></div><div class="grid"><section class="panel"><h2>${t('latestCases')}</h2>${caseRows(state.cases.slice(0,5))}</section><section class="panel"><h2>${t('wantedList')}</h2>${wantedRows(state.wanted.slice(0,6))}</section><section class="panel"><h2>${t('activeOfficers')}</h2>${officerStats()}</section></div>` }
